...
if "order_ratio" not in st.session_state:
    # ✅ RATIO-HR: params 저장값으로 초기화. fallback default 는 안전한 0.1(10%).
    try:
        ...
        st.session_state.order_ratio = (
            float(_p_ratio.order_ratio) if _p_ratio and _p_ratio.order_ratio else 0.1
        )
    except Exception:
        st.session_state.order_ratio = 0.1
