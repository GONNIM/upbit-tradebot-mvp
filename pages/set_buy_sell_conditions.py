...
# ✅ WO-7 (2026-09-18): 외부(HTS) 매수 진입가 처리 안내 (avg 계산 무변경 원칙)
st.info(
    "ℹ️ **외부(HTS) 매수 안내**: 봇이 HTS 매수를 감지하면 업비트 계정의 실제 "
    "`avg_buy_price` 값을 사용합니다. 이 값은 봇 감지 즉시 자동 반영되며 매도 "
    "필터(SL/TP/Trailing) 계산에 그대로 쓰입니다. 계산 방식(슬리피지·부분 체결 등)은 "
    "이번 라운드에서 변경되지 않습니다."
)
