...
## 📄 사용자 전달용 산출물 검증 규칙 (2026-09-13 신설)

**한 줄 원칙**: 사용자 전달용 PDF·이미지 산출물은 (1) **한글 글꼴을 파일에 임베드**하고 (2) **텍스트 추출 검증에 더해 실제 렌더(픽셀) 검증까지 통과**해야 전달한다.

**근거 사례 (2026-09-12)**: 안내문 PDF가 글꼴 미임베드로 사용자 환경에서 백지 표시. `pdftotext`류 텍스트 추출 검증은 통과했으나 실제 렌더 시 폰트 fallback 실패로 문자가 표시되지 않았다. 텍스트 추출만으로는 렌더 결함을 잡을 수 없다.

**검증 절차 (전달 전 필수)**:
1. **글꼴 임베드 확인**: `pdffonts <파일>` 결과에서 한글 글꼴이 `emb=yes, sub=yes` 상태인지 확인. `emb=no` 또는 시스템 폰트 참조만 있으면 다른 환경에서 백지.
2. **텍스트 추출 검증**: `pdftotext <파일> -` 로 추출한 텍스트가 원본 내용과 일치하는지 확인 (기존 검증).
3. **실제 렌더(픽셀) 검증**: `pdftoppm <파일> /tmp/render -png -r 150` 로 페이지를 이미지로 렌더한 뒤 파일을 실제로 열어 사용자 글자가 눈에 보이는지 확인. 이미지 파일 크기가 이례적으로 작으면(백지 = 압축률 급증) 의심.
4. **모바일 환경 확인**: 데스크톱만이 아니라 모바일 PDF 뷰어(카카오톡·텔레그램 첨부 미리보기 등)에서도 렌더 확인.

**금지 사항**:
- `pdftotext` 통과만으로 "PDF 이상 없음" 판정 금지 → 렌더 결함 놓침
- 시스템 폰트에 의존하는 PDF 생성 (예: HTML → PDF 변환 시 웹폰트 미포함) → 다른 OS·모바일에서 백지
- 사용자 회신 전에 산출물 자체를 열어보지 않고 전달
...
**마지막 업데이트**: 2026-09-13
**버전**: 2.6 (사용자 전달용 산출물 렌더 검증 규칙 추가)
