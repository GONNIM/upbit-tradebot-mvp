#!/bin/bash
# 매일 09:00 KST: 전일 24시간 거래/평가 요약 → Telegram
...
WINDOW="datetime('now', 'localtime', '-1 day')"
...
# 평균 수익률
AVG_PNL=$(q "SELECT printf('%+.2f%%', AVG(CASE WHEN entry_price > 0 THEN (price - entry_price) / entry_price * 100.0 ELSE NULL END)) FROM audit_trades WHERE type='SELL' AND timestamp >= $WINDOW;")
[ -z "$AVG_PNL" ] && AVG_PNL="(데이터 없음)"
...
LOG_COUNT=$(q "SELECT COUNT(*) FROM logs WHERE timestamp >= $WINDOW;")
BUY_EVAL="${BUY_EVAL:-0}"; SELL_EVAL="${SELL_EVAL:-0}"; LOG_COUNT="${LOG_COUNT:-0}"
...
🔍 평가 → 체결률
  ...
  SELL: ${SELL_EVAL}건 → ${SELL_F}건 (${SELL_RATE})
  로그: ${LOG_COUNT}건${ANOMALY}"

"$NOTIFIER" "📊 [일일 요약] tradebot" "$BODY"
