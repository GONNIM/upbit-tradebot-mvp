...
def engine_runner_main(
    ...
    try:
        # ✅ 파라미터 및 트레이더 설정
        base_params_path = f"{user_id}_{PARAMS_JSON_FILENAME}"
        params = load_params(base_params_path)
        # ✅ RATIO-HR: params_file 경로 주입 → 매수마다 order_ratio 최신값 로드
        # 2026-08-05 봉쇄: hot-reload가 base 파일을 읽어 EMA 저장값(0.1) 대신
        # 기본값(1.0)으로 100% 매수하던 결함 → scoped 통일.
        params_file_path = scoped_params_path(
            base_params_path, getattr(params, "strategy_type", None)
        )
        trader = UpbitTrader(
